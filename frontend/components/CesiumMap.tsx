...
const CesiumViewer: React.FC<CesiumMapProps> = ({
    ...
    useEffect(() => {
        ...
        // Province polygon click handler
        if (displayMode === 'polygon' && onMapClick && geoJsonDataSourceRef.current) {
            const Cesium = cesiumRef.current;
            const viewer = viewerRef.current;
                ...
                if (Cesium.defined(pickedObject) && pickedObject.id && pickedObject.id.polygon) {
                    ...
                    if (properties) {
                        properties.propertyNames.forEach((key: string) => {
                            propsObj[key] = properties.getValue(key);
                        });
                    }
                    onMapClick(propsObj);
                }
            }, Cesium.ScreenSpaceEventType.LEFT_CLICK);
            // Clean up handler on unmount or mode change
            return () => {
